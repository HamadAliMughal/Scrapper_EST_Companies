...
def get_search_results(keyword, state, max_results=13):
...
def scrape_contact_info(url, max_crawl=13):
    """Crawl a website and extract contact information, limiting to max_crawl pages."""
    visited = set()
    links_to_visit = [url]
    all_emails = []
    all_phones = []
    ...
    print(f"Finished crawling for {url}, crawled {crawl_count} pages.")
...
def main():
    ...
    for link in links:
        ...
        data.append({
            "Company_Name": link,  # Placeholder for company name
            "Emails": emails,
            "Phones": phones,
            "Website": link,
            "Address": state,
